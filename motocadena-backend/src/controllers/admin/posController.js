...
exports.addPayment = async (req, res, next) => {
  try {
    const sale_id = req.params.id;
    const { method, amount, currency, original_amount, bank } = req.body;
    if (!amount) throw makeError(400, 'amount es requerido');
    const insert = { sale_id, method: method || 'cash', amount, currency: currency || 'USD', original_amount: original_amount || amount, bank: bank || null };
